...
function LaunchLab({ notify, walletAddress, connectWallet }: { notify: (value: string) => void; walletAddress: string; connectWallet: () => Promise<void> }) {
  ...
  const [identity, setIdentity] = useState('An on-chain scout for people who want to spot early X Layer signals with their eyes open.')
  const [community, setCommunity] = useState('Signal Scouts')
  ...
  const styleKit = {
    'Signal core': { palette: ['#75a7ff', '#8b75ff', '#65dfbd'], line: 'A sharp signal mark for people who move with the chain, not the timeline.', angle: 'Signal before noise', voice: 'Focused, sharp, and transparent' },
    'Street poster': { palette: ['#ff875b', '#fa5fbf', '#ffd35f'], line: 'A loud, collectible rally cry designed for fast community recognition.', angle: 'The chain has eyes', voice: 'Bold, playful, and community-first' },
    'Clean protocol': { palette: ['#73dfc4', '#55a7ff', '#d5e2ff'], line: 'A calm, credible identity that makes the token feel intentional from day one.', angle: 'Simple tools. Clear signal.', voice: 'Clear, steady, and thoughtful' },
    'Retro arcade': { palette: ['#b887ff', '#6ce6ff', '#f6ed75'], line: 'A playful, high-energy identity with a repeatable visual language for posts.', angle: 'Press start on-chain', voice: 'Playful, energetic, and self-aware' },
  }[visualStyle]
  const displayName = name.trim() || 'Untitled token'
  const displaySymbol = symbol.trim() || 'TOKEN'
  const initials = displaySymbol.slice(0, 2)
  const launchPlan = () => { setPlanned(true); setSaved(false); notify('Launch Studio brief generated — planning and branding only') }
  const copyBrief = async () => {
    const brief = `${displayName} ($${displaySymbol})\nIdentity: ${identity}\nCommunity: ${community}\nSignature: ${signature}\nGoal: ${launchGoal}\nStyle: ${visualStyle}\nBrand voice: ${styleKit.voice}\nPositioning: ${styleKit.line}\nLaunch angle: ${styleKit.angle}\nGuardrails: publish disclosures, define liquidity policy, and never imply guaranteed returns.`
  ...
  const saveLaunchPlan = () => {
    const plan: LaunchPlan = { id: crypto.randomUUID(), name: displayName, symbol: displaySymbol, identity, community, signature, goal: launchGoal, style: visualStyle, supply, liquidity: liquidityUsd, creatorAllocation: creatorPercent, readiness: score, createdAt: new Date().toISOString() }
    try {
      const existing = JSON.parse(window.localStorage.getItem('nexus-launch-plans') || '[]') as LaunchPlan[]

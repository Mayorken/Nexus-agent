...
type View = 'radar' | 'desk' | 'launch' | 'activity' | 'pricing'
...
function App() {
  ...
  const nav = [[ 'radar', Compass, 'Alpha radar' ], [ 'desk', WalletCards, 'Trade desk' ], [ 'launch', Rocket, 'Launch lab' ], [ 'activity', Activity, 'Activity' ], [ 'pricing', CircleDollarSign, 'Pricing' ]] as const
...
function Pricing({ notify }: { notify: (value: string) => void }) { return <section className="alpha-content"><div className="page-heading"><p className="overline">FREE ACCESS / USAGE-BASED REVENUE</p><h1>Free to use. Fees only when value moves.</h1><p>No subscription wall for research, launch planning, or decision review. Nexus earns through transparent, disclosed transaction and creator-service fees.</p></div><div className="pricing-grid"><article className="pro"><span className="overline">NEXUS FREE</span><h2>$0 <small>/ month</small></h2><p>Everything needed to discover, review, simulate, and plan.</p><ul><li>Live X Layer signal scans</li><li>Trade-plan and quote review</li><li>Launch Lab simulations</li><li>Persistent audit history</li></ul><button onClick={() => notify('Nexus is free to use')}>Free for everyone</button></article><article><span className="overline">EXECUTION FEES</span><h2>Per swap</h2><p>A clearly disclosed service fee is shown before a user signs a real wallet transaction.</p><ul><li>No fee for research or quotes</li><li>No fee for demo trades</li><li>No hidden subscription</li><li>User signs every transaction</li></ul><button onClick={() => notify('Treasury wallet configuration is required before live fees can be enabled')}>Configure fee policy <ArrowRight size={15}/></button></article><article><span className="overline">CREATOR SERVICES</span><h2>Per launch</h2><p>Optional paid services for creators who need a polished public launch workflow.</p><ul><li>Brand and campaign assets</li><li>Liquidity and allocation review</li><li>Post-launch monitoring</li><li>Community reporting</li></ul><button onClick={() => notify('Creator services are planned as an optional usage-based fee')}>Explore creator services</button></article></div></section> }

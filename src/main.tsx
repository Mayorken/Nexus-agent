...
function Desk({ selected, price, change, amount, setAmount, units, ticker, status, setStatus, notify, onRecord, walletAddress, connectWallet }: { selected: Signal; price: number; change: number; amount: string; setAmount: (value: string) => void; units: number; ticker?: Ticker; status: 'idle' | 'preview' | 'complete'; setStatus: (value: 'idle' | 'preview' | 'complete') => void; notify: (value: string) => void; onRecord: (record: TradeRecord) => void; walletAddress: string; connectWallet: () => Promise<void> }) {
  ...
  useEffect(() => { setQuote(null); setQuoteError(''); setHandoff(null); setHandoffError(''); setApprovalHash(''); setApprovalReady(false); setSwapHash(''); setAnalysis(null); setAnalysisError('') }, [selected.tokenAddress, amount])
  ...
  const analyzeToken = async () => {
    ...
    try {
      const response = await fetch('/api/xlayer-analysis', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, token: { symbol: selected.symbol, name: selected.name, address: selected.tokenAddress, rank: selected.rank, price, change, liquidity: selected.liquidity, catalyst: selected.catalyst, thesis: selected.thesis, quote: quote ? { outputAmount: quote.outputAmount, outputSymbol: quote.outputSymbol, routeCount: quote.routeCount, priceImpactPercent: quote.priceImpactPercent, estimatedGasFee: quote.estimatedGasFee } : null } }) })
      const data = await response.json() as { analysis?: TokenAnalysis; message?: string }
      if (!response.ok || !data.analysis) throw new Error(data.message || 'Nexus could not analyze this token')
      setAnalysis(data.analysis); setStatus('preview')
